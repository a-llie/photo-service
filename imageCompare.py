...
def find_all_faces_in_image(imagePath):
    ...
    for face_location in face_locations:
        ...
        new_path = f"./public/saved_imgs/ref_img_{i}.jpg"
        i += 1
...
def create_face_encoding(cropped_img_path, original_image_path):
    try:
        ...
        face = Encoding(originalPath=original_image_path, originalImage=face_recognition.load_image_file(
            original_image_path), img=cropped_img_path, encoding=encoding)
        return face
    except:
        return None
...
def faces_from_input():
    ...
    while not file_path.is_file():
        path = input(
            "Can't find file, please check input and try again (0 to exit) \n >> ")
        if path == '0':
            exit(0)
        file_path = Path(path)
    ...
    ref_image = input(
        '\n./public/saved_imgs folder contains all the faces found in the photo. Please provide the path to desired individual (ex: ./public/saved_imgs/ref_img_0.jpg) \n >> ')
...
def main_deprecated():
    saved_images_path = './public/saved_imgs'
    stored_photos_path = './public/img'
